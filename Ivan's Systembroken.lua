...
if game.CoreGui:FindFirstChild("SysBroker") then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Ivan",Text = "GUI Already loaded, rejoin to re-execute",Duration = 5;})
	return
end
local version = 2
--VARIABLES
...
local function UpdateTarget(player)
	pcall(function()
		if table.find(ForceWhitelist,player.UserId) then
			SendNotify("Ivan","You cant target this player: @"..player.Name.." / "..player.DisplayName,5)
			player = nil
		end
	end)
	if (player ~= nil) then
...
CMDBar.FocusLost:Connect(function()
	...
	SendNotify("Ivan",("Executed "..command),5)
	CMDBar.Text = ""
end)
...
WalkSpeed_Button.MouseButton1Click:Connect(function()
	pcall(function()
		...
		SendNotify("Ivan","Walk speed updated.",5)
	end)
end)
...
JumpPower_Button.MouseButton1Click:Connect(function()
	pcall(function()
		...
		SendNotify("Ivan","Jump power updated.",5)
	end)
end)
...
FlySpeed_Button.MouseButton1Click:Connect(function()
	pcall(function()
		...
		SendNotify("Ivan","Fly speed updated.",5)
	end)
end)
...
SaveCheckpoint_Button.MouseButton1Click:Connect(function()
	...
	SendNotify("Ivan","Checkpoint saved.",5)
end)

ClearCheckpoint_Button.MouseButton1Click:Connect(function()
	SavedCheckpoint = nil
	SendNotify("Ivan","Checkpoint cleared.",5)
end)
...
WhitelistTarget_Button.MouseButton1Click:Connect(function()
	if TargetedPlayer ~= nil then
		if table.find(ScriptWhitelist, Players[TargetedPlayer].UserId) then
			...
			SendNotify("Ivan",TargetedPlayer.." removed from whitelist.",5)
		else
			table.insert(ScriptWhitelist, Players[TargetedPlayer].UserId)
			SendNotify("Ivan",TargetedPlayer.." added to whitelist.", 5)
		end
	end
end)
...
Day_Button.MouseButton1Click:Connect(function()
	if Shaders_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(255,0,0) then
		...
		SendNotify("Ivan","Please turn off shaders.",5)
	end
end)
...
FreeEmotes_Button.MouseButton1Click:Connect(function()
	if not FreeEmotesEnabled then
		...
		SendNotify("Ivan","Loading free emotes.\Made By sfc_ivan On Discord")
		loadstring(game:HttpGet("https://github.com/TyCanSing-ctrl/Libary/raw/refs/heads/main/IvansEmoteScript.lua"))()
	end
end)
...
Players.PlayerRemoving:Connect(function(player)
	pcall(function()
		if player.Name == TargetedPlayer then
			UpdateTarget(nil)
			SendNotify("Ivan","Targeted player left/rejoined.",5)
		end
	end)
end)
...
plr.CharacterAdded:Connect(function(x)
	...
	if PotionDi_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(PotionDi_Button)
		SendNotify("Ivan","PotionDick was automatically disabled due to your character respawn",5)
	end
	if PotionFling_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(PotionFling_Button)
		SendNotify("Ivan","PotionFling was automatically disabled due to your character respawn",5)
	end
	if AntiRagdoll_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(AntiRagdoll_Button)
		SendNotify("Ivan","AntiRagdoll was automatically disabled due to your character respawn",5)
	end
	if SpamMines_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(SpamMines_Button)
		SendNotify("Ivan","SpamMines was automatically disabled due to your character respawn",5)
	end
	if Fly_Button.Ticket_Asset.ImageColor3 == Color3.fromRGB(0,255,0) then
		ChangeToggleColor(Fly_Button)
		flying = false
		...
		KeyDownFunction:Disconnect()
		KeyUpFunction:Disconnect()
		SendNotify("Ivan","Fly was automatically disabled due to your character respawn",5)
	end
...
SendNotify("Ivan's SystemBroken Advanced","Advanced By Ivan",10)

print("Premuim Not Found!")
